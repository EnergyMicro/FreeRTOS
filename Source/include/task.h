...
/*
 * This functions returns the next tick a co routine is ready to run. If it returns
 * 0, then we can not go to sleep now. This might be because the xTickCount has
 * overflowed since the last time co routines where scheduled, or because
 * a co routine is ready to run now. 
 *
 * It takes in the current tickcount equal to xTickCount in task.c.
 */
portTickType xTaskNextTick(void);

/*
 * Called by tasks that want to restrcit which EM the MCU can go to.
 * For example: if vTaskCanGoToEM(1) is called, the MCU will not go to a em
 * lower then 1, untill the same task calls vTaskCanGoToEM(x) with x>1
 */
void vTaskCanGoToEM(unsigned portBASE_TYPE em);

/*
 * This function is used to get the number of times the xTickCount has overflowed.
 */
portBASE_TYPE lTaskGetNumberOfOverflows();

/*
 * This function returns the EM the MCU is allowed to go to.
 */
