...
#if configUSE_TICKLESSIDLE == 1
	PRIVILEGED_DATA static unsigned portBASE_TYPE pucTaksAbleToGoToEM[configENERGYMODES];
#endif
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	portTickType xTaskNextTick(void){
			/*If any tasks are ready to run, start from the top of idle loop again,
			and turn on interrupts. The fact that uxTopReadyPriority is used to check
			if any tasks are ready to run will ignore tasks sharing the priority of the
			idle task*/
			if(uxTopReadyPriority>0 || xPendingReadyList.uxNumberOfItems>0 
			   || pxReadyTasksLists[0].uxNumberOfItems>1){
				return 0;
			}
			/*Find out when the next task is to be woken up, and set tickNextEvent to
